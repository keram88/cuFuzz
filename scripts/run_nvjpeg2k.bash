...
#
# run_nvjpeg2k.bash - Run cuFuzz experiments on nvJPEG2000 library
#
# This script launches fuzzing campaigns for the NVIDIA nvJPEG2000 library using
...
#
# Usage:
#   ./run_nvjpeg2k.bash <MACHINE_ID> <APP_NAME> <RUN_ID> <TIME_SECONDS> <CONFIG>
#
# Arguments:
#   MACHINE_ID   - Identifier for the machine (used in log filenames)
#   APP_NAME     - Application name (typically "nvjpeg2k")
...
#   ./run_nvjpeg2k.bash node1 nvjpeg2k 1 86400 sansimtrace4hd
#
...
if [ "$CONFIG" = "nosanhd" ]; then
    AFL_NO_AFFINITY=1 CUFUZZ_MAP_SIZE=65536 AFL_MAP_SIZE=65536 AFL_SKIP_CPUFREQ=1 EDGE_COV=1 COUNT_WARP_LEVEL=1 COV_CTRL=3 AFL_PRELOAD=$CUFUZZ_PATH/src/cufuzz_cov.so ${CUFUZZ_PATH}/Tools/AFLplusplus/afl-fuzz -i $CUFUZZ_PATH/data/seeds/nvjpeg2000/ -o fuzz/${DSTN}/out_seeds_nosan_hd_${RUN} -V ${TIME} -t 120000 -x $CUFUZZ_PATH/data/dictionaries/jpeg2000.dict -Z ./nvjpeg2k_harness.afl.86 @@ 
fi

# nosanafl is the "AFL++" mode
if [ "$CONFIG" = "nosanafl" ]; then
    AFL_NO_AFFINITY=1 CUFUZZ_MAP_SIZE=65536 AFL_MAP_SIZE=65536 AFL_SKIP_CPUFREQ=1 ${CUFUZZ_PATH}/Tools/AFLplusplus/afl-fuzz -i $CUFUZZ_PATH/data/seeds/nvjpeg2000/ -o fuzz/${DSTN}/out_seeds_nosan_afl_${RUN} -t 120000 -V ${TIME} -Z -x $CUFUZZ_PATH/data/dictionaries/jpeg2000.dict ./nvjpeg2k_harness.afl.86 @@
fi

# sancovinc4hd is the "cufuzz-coverageIncrease" mode
if [ "$CONFIG" = "sancovinc4hd" ]; then
    AFL_NO_AFFINITY=1 AFL_SAN_ABSTRACTION=coverage_increase ASAN_APP="./nvjpeg2k_harness.asan.86" ORIGINAL_APP="./nvjpeg2k_harness.vanilla.86" SANITIZER_PATH=${CUDA_PATH:-/usr/local/cuda}/bin/compute-sanitizer SANITIZER_ARG="--tool=memcheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_RACE="--tool=racecheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_INIT="--tool=initcheck --report-api-errors=no --error-exitcode 99" CUFUZZ_MAP_SIZE=65536 AFL_MAP_SIZE=65536 AFL_SKIP_CPUFREQ=1 EDGE_COV=1 COUNT_WARP_LEVEL=1 COV_CTRL=3 AFL_PRELOAD=$CUFUZZ_PATH/src/cufuzz_cov.so ${CUFUZZ_PATH}/Tools/AFLplusplus/afl-fuzz -i $CUFUZZ_PATH/data/seeds/nvjpeg2000/ -o fuzz/${DSTN}/out_seeds_sancovinc_MRIA_hd_${RUN} -t 120000 -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_memcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_racecheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_initcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_asan.out -V ${TIME} -Z -x $CUFUZZ_PATH/data/dictionaries/jpeg2000.dict ./nvjpeg2k_harness.afl.86 @@
fi

# sanuniqtrace4hd is the "cufuzz-uniqueTrace" mode
if [ "$CONFIG" = "sanuniqtrace4hd" ]; then
    AFL_NO_AFFINITY=1 AFL_SAN_ABSTRACTION=unique_trace ASAN_APP="./nvjpeg2k_harness.asan.86" ORIGINAL_APP="./nvjpeg2k_harness.vanilla.86" SANITIZER_PATH=${CUDA_PATH:-/usr/local/cuda}/bin/compute-sanitizer SANITIZER_ARG="--tool=memcheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_RACE="--tool=racecheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_INIT="--tool=initcheck --report-api-errors=no --error-exitcode 99" CUFUZZ_MAP_SIZE=65536 AFL_MAP_SIZE=65536 AFL_SKIP_CPUFREQ=1 EDGE_COV=1 COUNT_WARP_LEVEL=1 COV_CTRL=3 AFL_PRELOAD=$CUFUZZ_PATH/src/cufuzz_cov.so ${CUFUZZ_PATH}/Tools/AFLplusplus/afl-fuzz -i $CUFUZZ_PATH/data/seeds/nvjpeg2000/ -o fuzz/${DSTN}/out_seeds_sanuniqtrace_MRIA_hd_${RUN} -t 120000 -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_memcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_racecheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_initcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_asan.out -V ${TIME} -Z -x $CUFUZZ_PATH/data/dictionaries/jpeg2000.dict ./nvjpeg2k_harness.afl.86 @@
fi
