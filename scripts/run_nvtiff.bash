...
#
# run_nvtiff.bash - Run cuFuzz experiments on nvTIFF library
#
# This script launches fuzzing campaigns for the NVIDIA nvTIFF library using
...
#
# Usage:
#   ./run_nvtiff.bash <MACHINE_ID> <APP_NAME> <RUN_ID> <TIME_SECONDS> <CONFIG>
#
# Arguments:
#   MACHINE_ID   - Identifier for the machine (used in log filenames)
#   APP_NAME     - Application name (typically "nvtiff")
...
#   ./run_nvtiff.bash node1 nvtiff 1 86400 sansimtrace4hd
#
...
if [ "$CONFIG" = "nosanhd" ]; then
    AFL_NO_AFFINITY=1 CUFUZZ_MAP_SIZE=65536 AFL_MAP_SIZE=65536 AFL_SKIP_CPUFREQ=1 EDGE_COV=1 COUNT_WARP_LEVEL=1 COV_CTRL=3 AFL_PRELOAD=$CUFUZZ_PATH/src/cufuzz_cov.so ${CUFUZZ_PATH}/Tools/AFLplusplus/afl-fuzz -i $CUFUZZ_PATH/data/seeds/nvtiff/ -o fuzz/${DSTN}/out_seeds_nosan_hd_${RUN} -V ${TIME} -t 120000  -x $CUFUZZ_PATH/data/dictionaries/tiff.dict -Z ./${APP}_harness.afl.86 @@
fi

# nosanafl is the "AFL++" mode
if [ "$CONFIG" = "nosanafl" ]; then
    AFL_NO_AFFINITY=1 CUFUZZ_MAP_SIZE=65536 AFL_MAP_SIZE=65536 AFL_SKIP_CPUFREQ=1 ${CUFUZZ_PATH}/Tools/AFLplusplus/afl-fuzz -i $CUFUZZ_PATH/data/seeds/nvtiff/ -o fuzz/${DSTN}/out_seeds_nosan_afl_${RUN} -t 120000 -V ${TIME} -x $CUFUZZ_PATH/data/dictionaries/tiff.dict -Z ./${APP}_harness.afl.86 @@ 
fi

# sancovinc4hd is the "cufuzz-coverageIncrease" mode
if [ "$CONFIG" = "sancovinc4hd" ]; then
    AFL_NO_AFFINITY=1 AFL_SAN_ABSTRACTION=coverage_increase ASAN_APP="./${APP}_harness.asan.86" ORIGINAL_APP="./${APP}_harness.vanilla.86" SANITIZER_PATH=${CUDA_PATH:-/usr/local/cuda}/bin/compute-sanitizer SANITIZER_ARG="--tool=memcheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_RACE="--tool=racecheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_INIT="--tool=initcheck --report-api-errors=no --error-exitcode 99" CUFUZZ_MAP_SIZE=65536 AFL_MAP_SIZE=65536 AFL_SKIP_CPUFREQ=1 EDGE_COV=1 COUNT_WARP_LEVEL=1 COV_CTRL=3 AFL_PRELOAD=$CUFUZZ_PATH/src/cufuzz_cov.so ${CUFUZZ_PATH}/Tools/AFLplusplus/afl-fuzz -i $CUFUZZ_PATH/data/seeds/nvtiff/ -o fuzz/${DSTN}/out_seeds_sancovinc_MRIA_hd_${RUN} -t 120000 -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_memcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_racecheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_initcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_asan.out -V ${TIME} -Z -x $CUFUZZ_PATH/data/dictionaries/tiff.dict ./${APP}_harness.afl.86 @@
fi

# sanuniqtrace4hd is the "cufuzz-uniqueTrace" mode
if [ "$CONFIG" = "sanuniqtrace4hd" ]; then
    AFL_NO_AFFINITY=1 AFL_SAN_ABSTRACTION=unique_trace ASAN_APP="./${APP}_harness.asan.86" ORIGINAL_APP="./${APP}_harness.vanilla.86" SANITIZER_PATH=${CUDA_PATH:-/usr/local/cuda}/bin/compute-sanitizer SANITIZER_ARG="--tool=memcheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_RACE="--tool=racecheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_INIT="--tool=initcheck --report-api-errors=no --error-exitcode 99" CUFUZZ_MAP_SIZE=65536 AFL_MAP_SIZE=65536 AFL_SKIP_CPUFREQ=1 EDGE_COV=1 COUNT_WARP_LEVEL=1 COV_CTRL=3 AFL_PRELOAD=$CUFUZZ_PATH/src/cufuzz_cov.so ${CUFUZZ_PATH}/Tools/AFLplusplus/afl-fuzz -i $CUFUZZ_PATH/data/seeds/nvtiff/ -o fuzz/${DSTN}/out_seeds_sanuniqtrace_MRIA_hd_${RUN} -t 120000 -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_memcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_racecheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_initcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_asan.out -V ${TIME} -Z -x $CUFUZZ_PATH/data/dictionaries/tiff.dict ./${APP}_harness.afl.86 @@
fi
...
if [ "$CONFIG" = "sansimtrace4hd" ]; then
    AFL_NO_AFFINITY=1 AFL_SAN_ABSTRACTION=simplify_trace ASAN_APP="./${APP}_harness.asan.86" ORIGINAL_APP="./${APP}_harness.vanilla.86" SANITIZER_PATH=${CUDA_PATH:-/usr/local/cuda}/bin/compute-sanitizer SANITIZER_ARG="--tool=memcheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_RACE="--tool=racecheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_INIT="--tool=initcheck --report-api-errors=no --error-exitcode 99" CUFUZZ_MAP_SIZE=65536 AFL_MAP_SIZE=65536 AFL_SKIP_CPUFREQ=1 EDGE_COV=1 COUNT_WARP_LEVEL=1 COV_CTRL=3 AFL_PRELOAD=$CUFUZZ_PATH/src/cufuzz_cov.so ${CUFUZZ_PATH}/Tools/AFLplusplus/afl-fuzz -i $CUFUZZ_PATH/data/seeds/nvtiff/ -o fuzz/${DSTN}/out_seeds_sansimtrace_MRIA_hd_${RUN} -t 120000 -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_memcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_racecheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_initcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_asan.out -V ${TIME} -Z -x $CUFUZZ_PATH/data/dictionaries/tiff.dict ./${APP}_harness.afl.86 @@
fi

# sansimtrace4afl is the "cufuzz-noDeviceCoverage" mode
if [ "$CONFIG" = "sansimtrace4afl" ]; then
    AFL_NO_AFFINITY=1 AFL_SAN_ABSTRACTION=simplify_trace ASAN_APP="./${APP}_harness.asan.86" ORIGINAL_APP="./${APP}_harness.vanilla.86" SANITIZER_PATH=${CUDA_PATH:-/usr/local/cuda}/bin/compute-sanitizer SANITIZER_ARG="--tool=memcheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_RACE="--tool=racecheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_INIT="--tool=initcheck --report-api-errors=no --error-exitcode 99" CUFUZZ_MAP_SIZE=65536 AFL_MAP_SIZE=65536 AFL_SKIP_CPUFREQ=1 ${CUFUZZ_PATH}/Tools/AFLplusplus/afl-fuzz -i $CUFUZZ_PATH/data/seeds/nvtiff/ -o fuzz/${DSTN}/out_seeds_sansimtrace_MRIA_afl_${RUN} -t 120000 -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_memcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_racecheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_initcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_asan.out -V ${TIME} -Z -x $CUFUZZ_PATH/data/dictionaries/tiff.dict ./${APP}_harness.afl.86 @@ 
fi

# perssansimtrace4hd is the "persistent" mode
if [ "$CONFIG" = "perssansimtrace4hd" ]; then
    AFL_NO_AFFINITY=1 COV_PERSISTENT=1 AFL_SAN_ABSTRACTION=simplify_trace ASAN_APP="./${APP}_harness.asan.86" ORIGINAL_APP="./${APP}_harness.vanilla.86" SANITIZER_PATH=${CUDA_PATH:-/usr/local/cuda}/bin/compute-sanitizer SANITIZER_ARG="--tool=memcheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_RACE="--tool=racecheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_INIT="--tool=initcheck --report-api-errors=no --error-exitcode 99" CUFUZZ_MAP_SIZE=65536 AFL_MAP_SIZE=65536 AFL_SKIP_CPUFREQ=1 EDGE_COV=1 COUNT_WARP_LEVEL=1 COV_CTRL=3 AFL_PRELOAD=$CUFUZZ_PATH/src/cufuzz_cov.so ${CUFUZZ_PATH}/Tools/AFLplusplus/afl-fuzz -i $CUFUZZ_PATH/data/seeds/nvtiff/ -o fuzz/${DSTN}/out_seeds_pers_sansimtrace_MRIA_hd_${RUN} -t 120000 -x $CUFUZZ_PATH/data/dictionaries/tiff.dict -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_persistent_memcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_persistent_racecheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_persistent_initcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_persistent_asan_A40.out  -V ${TIME} -Z ./${APP}_harness_persistent.afl.86 @@ 
fi

# sanall4hd is the "cufuzz-allTrace" mode
if [ "$CONFIG" = "sanall4hd" ]; then
    AFL_NO_AFFINITY=1 AFL_SAN_ABSTRACTION=all_trace ASAN_APP="./${APP}_harness.asan.86" ORIGINAL_APP="./${APP}_harness.vanilla.86" SANITIZER_PATH=${CUDA_PATH:-/usr/local/cuda}/bin/compute-sanitizer SANITIZER_ARG="--tool=memcheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_RACE="--tool=racecheck --report-api-errors=no --error-exitcode 99" SANITIZER_ARG_INIT="--tool=initcheck --report-api-errors=no --error-exitcode 99" CUFUZZ_MAP_SIZE=65536 AFL_MAP_SIZE=65536 AFL_SKIP_CPUFREQ=1 EDGE_COV=1 COUNT_WARP_LEVEL=1 COV_CTRL=3 AFL_PRELOAD=$CUFUZZ_PATH/src/cufuzz_cov.so ${CUFUZZ_PATH}/Tools/AFLplusplus/afl-fuzz -i $CUFUZZ_PATH/data/seeds/nvtiff/ -o fuzz/${DSTN}/out_seeds_sanall_MRIA_hd_${RUN} -t 120000 -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_memcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_racecheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_initcheck.out -w ${CUFUZZ_PATH}/src/cufuzz_sand/wrapper_asan.out -V ${TIME} -Z -x $CUFUZZ_PATH/data/dictionaries/tiff.dict ./${APP}_harness.afl.86 @@
fi
